...
- Who held which position before anyone saw anyone else's. Independent preparation reduces anchoring and correlated error. A record preserves declared positions and their provenance; it cannot by itself prove isolation or model identity. Review the supporting process evidence as well.
...
Agreement itself proves less than it appears to. Models from the same family, or models that have seen each other's output, agree for reasons that have nothing to do with being right. That is why the `independent-positions` claim requires two or more distinct providers and positions recorded before arbitration, and why the linter checks those structural prerequisites. It cannot establish that participants actually worked in isolation. Consensus is evidence to weigh, never an authority to obey.
