...
- `/imgs/og.png` and `docs/imgs/og.png` now exist, and `docs/index.html` is wired with OG/Twitter image metadata.
...
- Portfolio note: any other repo using the `docs/` + `docs/.well-known/` pattern on GitHub's legacy Jekyll build type may have the same silent `.well-known` 404 even with `.nojekyll` present. Worth a live check on repos with this pair.
